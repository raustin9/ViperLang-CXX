...
void HTTPServer::print_self() {
    ...
    for (const auto& key : m_controllers) {
        std::cout << key.first << "\n";
        key.second->test_print();
    }
}

/// @brief Route the received uri to the correct controller
/// @param uri The input uri that needs to be routed
void HTTPServer::route_uri(std::string& uri) {
